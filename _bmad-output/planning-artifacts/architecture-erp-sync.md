...
LMP Digital Services est un SaaS B2B/B2C (Spring Boot 3.5 + Angular) déployé sur Oracle Cloud via Dokploy. L'entreprise utilise ERPNext (v17-develop) comme back-office interne (comptabilité, CRM, projets, support). Les deux systèmes fonctionnent en silos — aucune synchronisation automatique n'existe.
...
| ERPNext — Webhooks | ❌ 0 configuré |
| ERPNext — Social Login Key | ❌ 0 configuré (pas de SSO) |
| ERPNext — API Keys | ❌ Non générées |
| ERPNext — Apps installées | frappe, erpnext, crm (Frappe CRM), lmp_branding, whitelabel |
...
- Frappe REST API : auth via `token api_key:api_secret`
- Frappe Webhooks : signature HMAC-SHA256 via `webhook_secret`
...
**Justification** : Le staff se connecte via SSO (auto-création User Frappe). Les clients ne se connectent jamais à l'ERP, donc leur Customer doit être créé via API quand ils s'inscrivent sur LMP.
...
2. Remplir :
   ...
   - **Redirect URL** : `https://crm.lmp-services.ca/api/method/frappe.integrations.oauth2_logins.login_via_oauth2`
...
|---|---|---|
| `ExternalSystemClient` | Client du système externe | ~~ERPNext~~ |
...
| `InboundWebhookController` | Récepteur de webhooks | ~~Frappe Webhook~~ |
