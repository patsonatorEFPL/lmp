package com.lmp.billing.service.processor;
...
import com.lmp.billing.service.PaymentProcessor;
...
import com.lmp.billing.dto.RefundRequestDto;
import com.lmp.billing.dto.RefundResponseDto;
...
public class StripeCheckoutPaymentProcessor implements PaymentProcessor {
        ...
        // Mapping des langues gtranslate vers les locales Stripe Checkout
        // 8 langues configurées : fr, en, es, de, it, nl, pt, lb
        private static final Map<String, Locale> GTRANSLATE_TO_STRIPE_LOCALE = Map.of(
                        "fr", Locale.FR,
                        "en", Locale.EN,
                        "es", Locale.ES,
                        "de", Locale.DE,
                        "it", Locale.IT,
                        "nl", Locale.NL,
                        "pt", Locale.PT_BR, // Stripe n'a pas "pt" seul, on utilise pt-BR
                        "lb", Locale.DE // Luxembourgeois non supporté par Stripe, fallback vers allemand
        );

        // Descriptions du produit dans chaque langue
        private static final Map<String, String> PRODUCT_DESCRIPTIONS = Map.of(
                        "fr", "Services de marketing digital LMP",
                        "en", "LMP Digital Marketing Services",
                        "es", "Servicios de marketing digital LMP",
                        "de", "LMP Digitale Marketingdienstleistungen",
                        "it", "Servizi di marketing digitale LMP",
                        "nl", "LMP Digitale marketingdiensten",
	...
	private String frontendUrl;
        ...
        public PaymentResponseDto processPayment(Order order, PaymentRequestDto paymentRequest)
                ...
                try {
                        ...
                        response.setProcessingFees(calculateProcessingFees(paymentRequest.getAmount(),
                                        paymentRequest.getCurrency()));
                        ...
                        logger.info("Stripe Checkout session created successfully - Session: {}",
                                        sessionResponse.getSessionId());
                        ...
                        logger.error("Stripe Checkout session creation failed for order {}: {}", order.getId(),
                                        e.getMessage(), e);
        ...
        public RefundResponseDto refundPayment(PaymentTransaction transaction, RefundRequestDto refundRequest)
                ...
                try {
                        ...
                        if (paymentIntentId == null) {
                                throw new PaymentProcessingException(
                                                "Impossible de trouver le PaymentIntent pour la session: "
                        ...
                        RefundCreateParams.Builder paramsBuilder = RefundCreateParams.builder()
                                        ...
                                        .setAmount(refundRequest.getAmount().multiply(new BigDecimal("100"))
                                                        .longValue())
                        ...
                        securityLogger.info(
                                        "Stripe refund processed successfully - Transaction: {}, Refund: {}, Status: {}",
         ...
         */
        public CheckoutSessionResponseDto createCheckoutSession(Order order, PaymentRequestDto paymentRequest)
                ...
                // Détecter la langue de l'utilisateur depuis les métadonnées (cookie googtrans)
                String userLang = resolveUserLanguage(paymentRequest);
                Locale stripeLocale = GTRANSLATE_TO_STRIPE_LOCALE.getOrDefault(userLang, Locale.FR);
                String productDescription = PRODUCT_DESCRIPTIONS.getOrDefault(userLang, PRODUCT_DESCRIPTIONS.get("fr"));

                logger.info("STRIPE_LOCALE_DEBUG - Detected language: '{}', Stripe locale: '{}', Description: '{}'",
                                userLang, stripeLocale, productDescription);
                ...
                metadata.put("user_language", userLang);
        ...
        private String buildCallbackUrl(String basePath, java.util.UUID orderId, String type) {
                // Use frontend URL for Stripe redirect callbacks (Angular SPA)
                String callbackBaseUrl = (frontendUrl != null && !frontendUrl.trim().isEmpty()) ? frontendUrl : baseUrl;
                String fullUrl = String.format("%s%s?order_id=%s&session_id={CHECKOUT_SESSION_ID}&type=%s",
                                callbackBaseUrl, basePath, orderId.toString(), type);

                // LOG DE DIAGNOSTIC : Tracer la construction des URLs de redirection
                logger.info("STRIPE_URL_DEBUG - Building {} URL for order {}: callbackBaseUrl='{}', basePath='{}', fullUrl='{}'",
                                type, orderId, callbackBaseUrl, basePath, fullUrl);
        ...
        private boolean isRetryableError(StripeException e) {
        	return e.getCode() != null && (e.getCode().equals("rate_limit") ||
        			e.getCode().equals("api_connection_error") ||
        			e.getCode().equals("api_error"));
        }
       
        /**
         * Résout la langue de l'utilisateur depuis les métadonnées du paiement.
         * La langue est transmise par le contrôleur qui lit le cookie googtrans de GTranslate.
         * Format du cookie googtrans : /fr/en (source/cible)
         *
         * @param paymentRequest la requête de paiement contenant les métadonnées
         * @return le code langue (fr, en, es, de, it, nl, pt, lb), par défaut "fr"
         */
        private String resolveUserLanguage(PaymentRequestDto paymentRequest) {
        	if (paymentRequest.getMetadata() != null && paymentRequest.getMetadata().containsKey("userLanguage")) {
        		String lang = String.valueOf(paymentRequest.getMetadata().get("userLanguage")).toLowerCase().trim();
        		if (GTRANSLATE_TO_STRIPE_LOCALE.containsKey(lang)) {
        			logger.info("STRIPE_LANG_RESOLVED - Language from metadata: '{}'", lang);
        			return lang;
        		}
        		logger.warn("STRIPE_LANG_UNKNOWN - Unknown language '{}', falling back to 'fr'", lang);
        	}
        	logger.info("STRIPE_LANG_DEFAULT - No language in metadata, using default 'fr'");
        	return "fr";
        }
       }
