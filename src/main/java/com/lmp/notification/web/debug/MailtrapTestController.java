...
import com.lmp.notification.service.MailtrapService;
...
/**
 * Contrôleur de test pour Mailtrap
 * 
 * Endpoints pour tester la configuration et l'envoi d'emails via Mailtrap SDK officiel
 * Uniquement disponible en mode développement
 */
// @RestController - Temporairement désactivé
// @RequestMapping("/api/test/mailtrap")
public class MailtrapTestController {

    private static final Logger logger = LoggerFactory.getLogger(MailtrapTestController.class);

        private final MailtrapService mailtrapService;


    public MailtrapTestController(MailtrapService mailtrapService) {
        this.mailtrapService = mailtrapService;
    }

    /**
     * Vérifie la configuration Mailtrap
     * GET /api/test/mailtrap/config
     */
    @GetMapping("/config")
    public ResponseEntity<Map<String, Object>> checkConfiguration() {
        logger.info("🔍 Vérification de la configuration Mailtrap");
        ...
        try {
            ...
            response.put("message", configured ? 
                "✅ Mailtrap configuré correctement" : 
                "⚠️ Mailtrap non configuré - Vérifier API token");
            
            logger.info("📊 Configuration Mailtrap: configuré={}, connexion={}", configured, connectionTest);
            
        } catch (Exception e) {
            logger.error("❌ Erreur lors de la vérification de configuration Mailtrap", e);
     ...
     * GET /api/test/mailtrap/connectivity
     */
    @GetMapping("/connectivity")
    public ResponseEntity<Map<String, Object>> testConnectivity() {
        logger.info("🔗 Test de connectivité Mailtrap");
        ...
        try {
            ...
            response.put("message", connectionOk ? 
                ...
                "❌ Échec de connexion à Mailtrap");
            
        } catch (Exception e) {
            logger.error("❌ Erreur lors du test de connectivité Mailtrap", e);
            response.put("success", false);
     ...
     * POST /api/test/mailtrap/send-test
     * 
    ...
    public ResponseEntity<Map<String, Object>> sendTestEmail(@RequestBody Map<String, String> request) {
        ...
        logger.info("📧 Envoi d'email de test Mailtrap vers: {}", email);
        ...
        try {
            ...
            response.put("message", "✅ Email de test envoyé avec succès via Mailtrap SDK officiel!");
            response.put("instructions", "Vérifiez votre boîte email et l'en-tête 'From' pour confirmer l'authentification du domaine");
            
            logger.info("✅ Email de test Mailtrap envoyé avec succès à {}", email);
     ...
     * POST /api/test/mailtrap/send-support
     * 
    ...
    public ResponseEntity<Map<String, Object>> sendSupportTestEmail(@RequestBody Map<String, String> request) {
        ...
        logger.info("📧 Envoi d'email support de test Mailtrap vers: {}", email);
        ...
        try {
            String htmlContent = String.format("""
                ...
                <hr>
                <p><em>Email envoyé via Mailtrap SDK officiel pour test</em></p>
            ...
            response.put("message", "✅ Email support de test envoyé avec succès via Mailtrap SDK officiel!");
            
            logger.info("✅ Email support de test Mailtrap envoyé avec succès à {}", email);
