...
public class AppointmentService {
    ...
    public Appointment createAppointment(AppointmentForm form, String userEmail) {
        ...
        checkTimeConflicts(form.getAppointmentDateTime(), form.getDurationMinutes());
        ...
        appointment.setAppointmentDate(form.getAppointmentDateTime());
    ...
    public Appointment updateAppointment(java.util.UUID appointmentId, AppointmentForm form) {
        ...
        if (!appointment.getAppointmentDate().equals(form.getAppointmentDateTime()) ||
            !appointment.getDurationMinutes().equals(form.getDurationMinutes())) {
            checkTimeConflicts(form.getAppointmentDateTime(), form.getDurationMinutes(), appointmentId);
        }
    ...
    private void validateAppointmentForm(AppointmentForm form) {
        ...
        if (form.getAppointmentDateTime().isBefore(now.plusHours(24))) {
        ...
        LocalDate appointmentDate = form.getAppointmentDateTime().toLocalDate();
        ...
        for (int hour : availableHours) {
            ...
            if (date.equals(LocalDate.now())) {
                if (slotStart.isBefore(now) || slotStart.equals(now)) {
                    logger.debug("Créneau passé ou en cours ignoré : {}h (heure actuelle: {})", 
                ...
                if (slotStart.isBefore(now.plusHours(1))) {
                    logger.debug("Créneau trop proche ignoré (moins d'1h d'avance) : {}h", hour);
                    continue;
                }
            }
    ...
    private void invalidateCacheForDate(LocalDate date) {
        ...
        if (slotsCache.remove(cacheKey) != null) {
            logger.debug("Cache invalidé pour la date {}", date);
        }
    }
    ...
    public void clearSlotsCache() {
        slotsCache.clear();
        logger.info("Cache des créneaux vidé complètement");
    }
