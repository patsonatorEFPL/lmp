...
public class SyncCallbackService {
    ...
    /**
     * Pour le DocType ERPNext "User", la clé primaire est l'email lui-même.
    ...
    private void updateUserExternalErpUserId(UUID userId, String externalId) {
        userRepository.findById(userId).ifPresentOrElse(
                user -> {
                    ...
                    userRepository.save(user);
                    log.info("🔗 [CALLBACK] User {} linked to external ERPNext User {}", userId, externalId);
                },
                () -> log.warn("⚠️ [CALLBACK] User {} not found for ERPNext User link", userId)
        );
    }
    ...
    private void enqueueInvoiceIfNeeded(Order order) {
        ...
        try {
            ...
            // Sans so_detail, ERPNext ne met pas à jour per_billed sur le Sales Order.
     ...
     * pour que ERPNext puisse calculer per_billed sur le Sales Order.
     */
    @SuppressWarnings("unchecked")
     ...
     *       avec {@code payment_term} sur la référence SINV pour que ERPNext mette à jour
     ...
     * calculés par ERPNext (évite les écarts d'arrondi entre LMP et l'ERP).
     */
    @SuppressWarnings("unchecked")
     ...
     * Retourne un map payment_term → payment_amount (montant exact calculé par ERPNext).
     */
    @SuppressWarnings("unchecked")
    ...
    private void clearUserExternalErpUserId(UUID userId) {
        userRepository.findById(userId).ifPresent(user -> {
            ...
            userRepository.save(user);
            log.info("🧹 [CALLBACK] Cleared external ERPNext User ID on User {}", userId);
        });
    }
