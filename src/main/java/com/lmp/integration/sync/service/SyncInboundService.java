...
public class SyncInboundService {
    ...
    private void routeToHandler(SyncEntityType entityType, InboundSyncPayload payload) {
        // Frappe émet "on_trash" quand un document est supprimé
        ...
        switch (entityType) {
            ...
            case ERP_USER -> userRepository.findByExternalErpUserId(externalId).ifPresentOrElse(
                    user -> {
                        ...
                        userRepository.save(user);
                        log.info("🗑️ [SYNC IN] Unlinked User '{}' (external ERPNext User '{}' deleted)",
                                user.getEmail(), externalId);
                    },
                    () -> log.debug("📋 [SYNC IN] No User found for external ERPNext User '{}' — ignoring", externalId)
            );
