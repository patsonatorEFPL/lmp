...
window.AppointmentModal = {
    ...
    open: function(options = {}) {
        ...
        // Pré-remplir les champs si des options sont fournies
...
const AppointmentModalController = {
    ...
    function renderCalendar() {
        ...
        // Gérer la navigation (délégation d'événements)
        $(document).off('click', '#prevMonth, #nextMonth');
        $(document).on('click', '#prevMonth', function() {
            ...
            if (currentMonthDate > minDate) {
                ...
                log(`⬅️ Mois précédent : ${monthNames[currentMonth]} ${currentYear}`);
                renderCalendar();
            }
        });
        ...
        $(document).on('click', '#nextMonth', function() {
            ...
            log(`➡️ Mois suivant : ${monthNames[currentMonth]} ${currentYear}`);
            renderCalendar();
        });
    }
    ...
    function selectDate(date) {
        ...
        log(`📌 Date sélectionnée : ${formatDate(date)}`, 'success');
        
        // Mettre à jour l'interface
        $('.calendar-day').removeClass('selected');
        $(`.calendar-day[data-date="${formatDate(date)}"]`).addClass('selected');
        $('#appointmentDate').val(formatDate(date));
    ...
    // Générer les créneaux horaires (heures pleines uniquement)
    function generateTimeSlots(date) {
        log(`🕐 Génération locale des créneaux pour ${formatDate(date)}`);
        ...
        if (dayOfWeek >= 1 && dayOfWeek <= 5) {
            // Heures disponibles : 9h, 10h, 11h, 13h, 14h, 15h, 16h
            ...
            for (let hour of availableHours) {
                ...
                if (isToday) {
                    // Ignorer les heures passées ou trop proches (moins d'1h d'avance)
                    if (hour <= currentHour + 1) {
                        log(`⏭️ Créneau ignoré (passé ou trop proche) : ${hour}:00`, 'warning');
                        continue;
                    }
                }
                ...
                slots.push(slot);
                log(`✅ Créneau ajouté : ${slot}`, 'success');
            }
        } else {
            log('🚫 Week-end détecté - aucun créneau disponible', 'warning');
        }
        
        log(`📊 Total : ${slots.length} créneaux générés localement`);
        return slots;
    }
    ...
    function loadAvailableSlots(date) {
        ...
        // Si c'est dans le passé, pas de créneaux
        const today = new Date();
        today.setHours(0,0,0,0);
        if (date < today) {
            log('⛔ Date dans le passé - aucun créneau', 'warning');
            displayTimeSlots([]);
            return;
        }
        
        // Si c'est un week-end, pas de créneaux
        if (date.getDay() === 0 || date.getDay() === 6) {
            log('🚫 Week-end - aucun créneau', 'warning');
            displayTimeSlots([]);
            return;
        }
    ...
    // ============= GESTION DU RÉSUMÉ =============
        ...
        $('#appointmentName, #appointmentService').on('input change', updateSummary);
    ...
    $('#appointmentForm').on('submit', function(e) {
        ...
        const formData = {
            ...
            message: $('#appointmentMessageField').val()
        };
