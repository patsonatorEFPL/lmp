...
function showRegistrationModal(serviceName, amount, currency) {
    ...
    document.getElementById('orderSummaryAmount').textContent =
...
async function handleRegisterAndCheckout(event) {
    ...
    try {
        ...
        console.log('DEBUG: Envoi des données d\'inscription avec commande intégrée:', registrationData);
        ...
        console.log('DEBUG: Préparation requête sans CSRF (endpoint configuré pour ignorer CSRF)');
        ...
        if (response.ok && result.success) {
            ...
            showMessage('successMessage', result.message || 'Inscription réussie ! Redirection vers le paiement...');
            ...
            if (result.orderId) {
                setTimeout(async () => {
                    try {
                        console.log('DEBUG: Création de session Stripe pour commande:', result.orderId);
                        await createStripeSession(result.orderId);
                    } catch (error) {
                        console.error('Erreur lors de la création de session Stripe:', error);
                        showMessage('errorMessage', 'Erreur lors de la redirection vers le paiement : ' + error.message);
        ...
        showMessage('errorMessage', 'Erreur de connexion. Veuillez vérifier votre connexion internet et réessayer.');
        setLoadingState(false);
    }
}
...
function validateForm(formData) {
    ...
    if (!formData.get('acceptTerms')) {
        showMessage('errorMessage', 'Vous devez accepter les conditions d\'utilisation');
        return false;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('keydown', function(event) {
        if (event.key === 'Escape' && !document.getElementById('bookingModal').classList.contains('hidden')) {
            closeBookingModal();
        }
    });
});
...
function handleBookingSubmit(event) {
    console.warn('handleBookingSubmit is deprecated, use handleRegisterAndCheckout instead');
...
function toggleBillingAddress() {
    ...
    if (billingFields && toggleText && toggleIcon) {
        if (billingFields.classList.contains('hidden')) {
            ...
            toggleText.textContent = '+ Ajouter une adresse de facturation (optionnel)';
            toggleIcon.classList.remove('fa-chevron-up');
            toggleIcon.classList.add('fa-chevron-down');
            ...
            const companyNameField = document.querySelector('input[name="companyName"]');
 ...
 */
async function handleAuthenticatedUserOrder(serviceName, amount, currency = 'CAD') {
    console.log('DEBUG: Création de commande pour utilisateur connecté:', {
        service: serviceName,
        amount: amount,
        currency: currency,
    ...
    try {
        ...
        // Créer une commande temporaire via l'API pour utilisateurs connectés
        const orderData = {
        ...
        console.log('DEBUG: Envoi de la requête de création de commande...');
        
        const response = await fetch('/api/orders/create-temp', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-Requested-With': 'XMLHttpRequest',
        ...
        if (response.ok && result.id) {
            console.log('DEBUG: Commande créée avec succès:', result.id);
            // Rediriger directement vers Stripe Checkout
            await createStripeSession(result.id);
        } else {
