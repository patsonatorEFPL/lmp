...
/**
 * Vérifie que InstallmentCalculator produit exactement les mêmes montants qu'ERPNext.
 * Valeurs de référence extraites de l'API ERPNext sur SINV-2026-00029 (1200 € en 3x).
 */
class InstallmentCalculatorTest {

    @Test
    void threeInstallments_1200_matchesErpNext() {
        // Valeurs réelles ERPNext pour "Paiement en 3x" sur 1200 €
        List<InstallmentCalculator.Installment> result = InstallmentCalculator.calculate(
                new BigDecimal("1200.00"), 3);

        assertEquals(3, result.size());

        // 1ère échéance: 33.33% → 399.96 (ERPNext: 399.96)
        ...
        // 2ème échéance: 33.33% → 399.96 (ERPNext: 399.96)
        ...
        // 3ème échéance: 33.34% → 400.08 (ERPNext: 400.08)
    ...
    void threeInstallments_100_erpRounding() {
        // 100 € / 3 → ERPNext: 33.33% × 100 = 33.33, 33.33, 33.34
        List<InstallmentCalculator.Installment> result = InstallmentCalculator.calculate(
