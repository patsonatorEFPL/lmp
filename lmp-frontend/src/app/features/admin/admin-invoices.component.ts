    ...
    <div class="flex h-full flex-col overflow-hidden">
      ...
      <div class="flex items-center justify-between gap-2 px-5 py-4">
        <div class="flex items-center gap-2">
          <lucide-icon [img]="ReceiptIcon" [size]="18" class="text-zinc-500 dark:text-zinc-400"></lucide-icon>
          <h2 class="text-base font-semibold text-zinc-900 dark:text-zinc-100">Factures</h2>
        </div>
      ...
      @if (showFilterPanel()) {
        <div class="flex items-center gap-2 border-b border-zinc-100 px-5 pb-3 dark:border-zinc-800">
          ...
          >
            ...
            <option value="PARTLY_PAID">Partiellement payée</option>
          </select>
        </div>
      }
      ...
      <div class="flex-1 overflow-auto px-3 sm:px-5">
        @if (loading()) {
          ...
          <div class="mb-2 flex min-w-max items-center rounded-lg bg-zinc-100 py-1.5 text-sm font-normal leading-none text-zinc-500 dark:bg-zinc-800/70 dark:text-zinc-400">
            <div class="w-32 shrink-0 px-2">N° facture</div>
...
export class AdminInvoicesComponent implements OnInit {
  ...
  readonly filteredItems = signal<InvoiceItem[]>([]);
  ...
  getStatusLabel(status: string): string {
    const labels: Record<string, string> = {
      ...
      UNPAID: 'Non payée',
      OVERDUE: 'En retard',
      PARTLY_PAID: 'Partiellement payée',
    };
